...
async def get_total_sales_for_given_time_period(db:Session = Depends(get_db), current_user = Depends(oauth2.get_current_user),
                                          range: Annotated[Optional[str], Query(description="Filter: day, week, month, year")] = None,
...
async def get_none_sales_over_given_period(
        range: Annotated[Optional[str], Query(description="Filter: day, week, month, year")] = None,
        date_value: Annotated[Optional[date], Query(description="reference_date (YYYY-MM-DD)")] = None,
        db: Session = Depends(get_db),
        current_user=Depends(oauth2.get_current_user)
...
async def get_total_sales_for_day_for_specific_item(
    id:int,
